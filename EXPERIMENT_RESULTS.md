...
This file records returned Runpod results and terminal failed-run outcomes.
...
or is explicitly stopped as a failed run.
...
| E130 Hodge-centered boundary readout | runtime failed | - | - | - | - | - | - | Stopped after roughly 3 hours with no step-9000 history, result bundle, eval details, or checkpoint; history remained at inherited E128 step 8500. Treat static Hodge double-centering as a runtime-failed branch; the no-Hodge E138 fallback is recorded separately. |
| E138 no-Hodge face-cyclic boundary readout | runtime failed | - | - | - | - | - | - | Stopped at the documented no-write cutoff after `03:00:56` elapsed on owned Runpod pod `c67fbk189vnvfp`: startup log and artifacts remained unchanged since `2026-05-15T06:02Z`, history still ended at inherited E128 step 8500, and no `results.json`, `results.csv`, eval details, or checkpoint existed. Trace was preserved locally and only E138 PID `24980` was stopped; use the no-Hodge E139 oriented boundary-cochain fallback. |
| E139 no-Hodge oriented boundary cochain | runtime failed | - | - | - | - | - | - | Stopped after `00:45:26` elapsed on owned Runpod pod `c67fbk189vnvfp` because it remained in the same no-write first-step state as E130/E138 while burning `09:08:39` process CPU time: artifacts contained only startup `run_metadata.json` and inherited E128 step-8500 history, with no result bundle, eval details, checkpoint, or status file. Trace was preserved locally and only E139 PIDs `42517` / `42514` were stopped; pivot to E140 selected-boundary realization anti-collapse. |
