...
def test_model_inputs_add_training_only_simplex_curricula():
    ...
    training_config = TrainingConfig(
        ...
        simplex_signed_tetra_to_face_runtime_scale_final=0.2,
        simplex_signed_tetra_to_face_runtime_scale_ramp_start_step=10,
        simplex_signed_tetra_to_face_runtime_scale_ramp_steps=10,
        ...
        simplex_boundary_edge_star_residual_runtime_scale_ramp_start_step=10,
        simplex_boundary_edge_star_residual_runtime_scale_ramp_steps=10,
        simplex_boundary_oriented_cochain_runtime_scale=0.0,
        simplex_boundary_oriented_cochain_runtime_scale_final=0.25,
        simplex_boundary_oriented_cochain_runtime_scale_ramp_start_step=10,
        simplex_boundary_oriented_cochain_runtime_scale_ramp_steps=10,
    ...
    assert "simplex_boundary_edge_star_residual_scale_override" not in eval_inputs
    assert "simplex_boundary_oriented_cochain_scale_override" not in eval_inputs
    assert "simplex_boundary_face_cyclic_readout_scale_override" not in eval_inputs
    ...
    assert torch.allclose(
        train_inputs["simplex_boundary_edge_star_residual_scale_override"],
        torch.tensor(0.125),
    )
    assert torch.allclose(
        train_inputs["simplex_boundary_oriented_cochain_scale_override"],
        torch.tensor(0.125),
    )
    assert torch.allclose(
...
def test_trainer_cli_accepts_simplex_star_context_overrides():
    ...
    assert args.simplex_boundary_oriented_cochain_runtime_scale_ramp_start_step == 6000
...
def test_simplicial_hodge_face_update_adds_no_parameters():
    ...
    hodge_medium = replace(
        ...
        simplex_signed_tetra_to_face_scale=0.25,
    )
...
def test_simplicial_triangle_attention_bias_stays_inside_medium_budget():
    af2_medium = replace(load_model_config("medium"), use_simplicial_evoformer=False)
    simplex_medium = load_model_config("simplexfold_medium_param_matched")
    triangle_bias_medium = replace(
        simplex_medium,
        simplex_use_msa_to_face=True,
        simplex_face_top_k=24,
        simplex_tetra_top_k=48,
    ...
    af2_params = sum(parameter.numel() for parameter in AlphaFold2(af2_medium).parameters())
    triangle_bias_params = sum(parameter.numel() for parameter in AlphaFold2(triangle_bias_medium).parameters())

    assert af2_params == 3_106_642
    assert triangle_bias_params == 3_203_186
...
def test_triangle_attention_bias_runs_evoformer_block_eagerly(monkeypatch):
    import minalphafold.model as model_module
    from minalphafold.evoformer import SimplicialEvoformer

    model_config = replace(load_model_config("tiny"), simplex_triangle_attention_bias_scale=0.05)
    model = AlphaFold2(model_config)
    model.train()
